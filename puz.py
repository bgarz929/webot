import hashlib
import base64
import itertools
from Crypto.Cipher import AES
from Crypto.Util.Padding import unpad

# 1. Teks Cipher asli dari puzzle.html
msg = ""

# 2. Parsing format OpenSSL "Salted__"
ciphertext_bytes = base64.b64decode(msg)
salt = ciphertext_bytes[8:16] # Ambil 8 byte salt
actual_ciphertext = ciphertext_bytes[16:] # Sisanya adalah chipertext murni

# 3. Replika fungsi EvpKDF milik CryptoJS (10.000 iterasi dengan MD5)
def evpkdf_cryptojs(passphrase: bytes, salt: bytes, key_size: int, iv_size: int, iterations: int):
    target_len = key_size + iv_size
    derived = b''
    block = None
    while len(derived) < target_len:
        h = hashlib.md5()
        if block:
            h.update(block)
        h.update(passphrase)
        h.update(salt)
        block = h.digest()
        
        # CryptoJS mengulang MD5 sebanyak sisa iterasi
        for _ in range(1, iterations):
            block = hashlib.md5(block).digest()
        derived += block
    
    return derived[:key_size], derived[key_size:target_len]

# 4. Fungsi loop SHA512 (sama seperti di JavaScript)
def hash_loop(password):
    h = hashlib.sha512(password.encode('utf-8')).digest()
    for _ in range(11512):
        h = hashlib.sha512(h).digest()
    # PENTING: JS i.toString() menghasilkan string HEX!
    return h.hex()

def try_decrypt(pwd):
    passphrase_hex = hash_loop(pwd)
    
    # 32 byte Key, 16 byte IV, 10000 Iterations
    key, iv = evpkdf_cryptojs(passphrase_hex.encode('utf-8'), salt, 32, 16, 10000)
    
    cipher = AES.new(key, AES.MODE_CBC, iv)
    try:
        pt = cipher.decrypt(actual_ciphertext)
        pt_unpad = unpad(pt, AES.block_size)
        
        # Validasi jika berhasil terbuka
        if b'"kty"' in pt_unpad or b'RSA' in pt_unpad:
            return pt_unpad
    except Exception:
        return None
    return None

# Panel kata (384 Kombinasi Total)
panel_words = [
    ["wand","anid","sand","wind","rand","arwv","seed","bind","knot","cord"],
    ["hash","code","perm","sort"],
    ["shax","sh25","sha2","sh38","sha3","s256","s384"],
    ["swap","exch","trad","fiat"],
    ["eire","scan","nord","oslo"],
    ["tree","wood","root","seed"],
    ["aust"],
    ["dots","node","peer","mesh","dust"]
]

print("Memulai pencarian otomatis dari algoritma CryptoJS yang diperbaiki...")
for combo in itertools.product(*panel_words):
    password = "".join(combo)
    
    result = try_decrypt(password)
    if result:
        print(f"\n[!!!] KUNCI BERHASIL DITEMUKAN: {password}")
        print("-" * 50)
        try:
            print(result.decode('utf-8', errors='ignore')[:500] + "\n\n... (Sisa JSON diabaikan)")
        except:
            print("Data biner ditemukan.")
            
        with open("arweave_keyfile.json", "wb") as f:
            f.write(result)
            print("=> File arweave_keyfile.json berhasil disimpan!")
        break
