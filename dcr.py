...
password = "wandhashs384exchnordseedaustdust"
...
# 1. SHA512 loop seperti CryptoJS
h = hashlib.sha512(password.encode()).digest()
for _ in range(11512):
    h = hashlib.sha512(h).digest()

passphrase = h.hex().encode()

# 2. decode base64 ciphertext
data = base64.b64decode(msg)

# 3. OpenSSL salted format
assert data[:8] == b"Salted__"
salt = data[8:16]
ciphertext = data[16:]

# 4. EvpKDF (CryptoJS compatible)
key_iv = hashlib.pbkdf2_hmac("sha1", passphrase, salt, 10000, 48)
key = key_iv[:32]
iv = key_iv[32:48]
...
# 6. hex → ascii (hex2a)
hex_text = pt.decode()
wallet = bytes.fromhex(hex_text).decode()
