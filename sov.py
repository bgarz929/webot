...
def hash_loop(password: str):
    h = sha512(password.encode()).digest()
    for _ in range(11512):
        h = sha512(h).digest()
    return h.hex()

def evpkdf(passphrase: bytes, salt: bytes):
    key_iv = b''
    block = None
...
def try_decrypt(pwd):
    ...
    passphrase = hash_loop(pwd).encode()
...
def main():
    ...
    passwords = ("".join(p) for p in itertools.product(*panel_words))

    batch_size = 5000

    batches = chunk_generator(passwords, batch_size)
