...
# 2. Parsing format OpenSSL "Salted__"
ciphertext_bytes = base64.b64decode(msg)
salt = ciphertext_bytes[8:16]                 # 8 byte salt
actual_ciphertext = ciphertext_bytes[16:]     # sisanya ciphertext murni

# 3. Replika fungsi EvpKDF milik CryptoJS (10.000 iterasi dengan MD5)
def evpkdf_cryptojs(passphrase: bytes, salt: bytes, key_size: int, iv_size: int, iterations: int):
    target_len = key_size + iv_size
    derived = b''
    block = None
    while len(derived) < target_len:
        ...
        derived += block
    return derived[:key_size], derived[key_size:target_len]

# 4. Fungsi loop SHA512 (sama seperti di JavaScript)
def hash_loop(password):
    h = hashlib.sha512(password.encode('utf-8')).digest()
    for _ in range(11512):
        h = hashlib.sha512(h).digest()
    return h.hex()  # JS i.toString() menghasilkan string HEX!

def try_decrypt(pwd):
    passphrase_hex = hash_loop(pwd)
    key, iv = evpkdf_cryptojs(passphrase_hex.encode('utf-8'), salt, 32, 16, 10000)
    cipher = AES.new(key, AES.MODE_CBC, iv)
    try:
        pt = cipher.decrypt(actual_ciphertext)
        pt_unpad = unpad(pt, AES.block_size)
        # Kriteria persis seperti JavaScript: mencari substring b'"kty":"RSA"'
        if b'"kty":"RSA"' in pt_unpad:
            return pt_unpad
    except Exception:
        return None
    return None
...
def worker(password):
    # Counter lokal per proses
    worker.counter = getattr(worker, "counter", 0) + 1
    if worker.counter % 10000 == 0:
        print(f"[PID:{os.getpid()}][{worker.counter}] Mencoba: {password}", flush=True)
...
panel_words = [
    ...
    ["hash","code","perm","sort"],
    ["shax","sh25","sha2","sh38","sha3","s256","s384"],
    ["swap","exch","trad","fiat"],
    ["eire","scan","nord","oslo"],
    ["tree","wood","root","seed"],
    ["aust"],
    ["dots","node","peer","mesh","dust"]
]
...
def main():
    ...
    print(f"Memulai pencarian paralel ({total} kombinasi) dengan {num_workers} core...")
    print("Kriteria: b'\"kty\":\"RSA\"' dalam hasil dekripsi")
        ...
        for res in pool.imap_unordered(worker, passwords_gen):
            if res is not None:
                ...
                try:
                    ...
                    print("Data biner ditemukan.")
                filename = "arweave_keyfile_wHP6OPG5GMF5dedo_CD8AAy6x8La-gfI5b5pk65Tx_0.json"
            ...
            print("Tidak ditemukan password yang cocok.")
